...
def get_pie_chart(entered_site):
    if entered_site == 'ALL':
        # Use only the successful launches (class==1)
        ...
        # Filter on only the selected site
        filtered_df = spacex_df[spacex_df['Launch Site'] == entered_site]
        class_counts = filtered_df['class'].value_counts().reset_index() # Get the counts of 0 and 1 values
...
def get_scatter_chart(entered_site, entered_payload_range):
    ...
    if entered_site == 'ALL':
        # Filter on selected payload range
